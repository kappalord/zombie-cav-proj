...
{
	local PlatformInterfaceWebResponse R;
	local int i;
	local string H, V;

	R = PlatformInterfaceWebResponse(Result.Data.ObjectValue);
	`log("Response Original URL " $ R.OriginalURL);
	`log("Content-Type = '" $ R.GetHeaderValue("Content-Type") $ "'"); 
	`log("Response[" $ R.Tag @ R.ResponseCode $ "] = ");
	if (R.BinaryResponse.length > 0)
	{
		`log("   " $ R.BinaryResponse.length $ " bytes of binary data");
	}
	else
	{
		`log("   " $ R.StringResponse);
	}

	for (i = 0; i < R.GetNumHeaders(); i++)
	{
		R.GetHeader(i, H, V);
		`log("Header: " $ H @ V);
	}
